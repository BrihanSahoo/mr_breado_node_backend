...
app.get('/',(req,res)=>res.json({success:true,message:'Mr Breado Node backend running',apiPrefix,version:'payment-auth-fix-v11'}));
app.get(`${apiPrefix}/health`,(req,res)=>res.json({success:true,message:'OK',version:'payment-auth-fix-v11',time:new Date().toISOString()}));
app.get(`${apiPrefix}/version`,(req,res)=>res.json({success:true,version:'payment-auth-fix-v11',paymentCreateOrder:'public-direct-route-null-safe'}));
...
app.use((req,res)=>res.status(404).json({success:false,message:'Endpoint not found',path:req.originalUrl,version:'payment-auth-fix-v11'})); app.use(error); module.exports=app;
