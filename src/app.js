...
app.get('/',(req,res)=>res.json({success:true,message:'Mr Breado Node backend running',apiPrefix,version:'commerce-v19'}));
app.get(`${apiPrefix}/health`,(req,res)=>res.json({success:true,message:'OK',version:'commerce-v19',time:new Date().toISOString()}));
app.get(`${apiPrefix}/version`,(req,res)=>res.json({success:true,version:'commerce-v19',paymentCreateOrder:'public-direct-route-null-safe',featureUpgrade:'v19',realData:'enabled',commerce:'seller-admin-live-orders-real-data-fixed'}));
...
app.use((req,res)=>res.status(404).json({success:false,message:'Endpoint not found',path:req.originalUrl,version:'commerce-v19'})); app.use(error); module.exports=app;
