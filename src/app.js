...
app.get('/',(req,res)=>res.json({success:true,message:'Mr Breado Node backend running',apiPrefix,version:'auth-direct-v21'}));
app.get(`${apiPrefix}/health`,(req,res)=>res.json({success:true,message:'OK',version:'auth-direct-v21',time:new Date().toISOString()}));
app.get(`${apiPrefix}/version`,(req,res)=>res.json({success:true,version:'auth-direct-v21',paymentCreateOrder:'public-direct-route-null-safe',featureUpgrade:'v21',realData:'enabled',commerce:'direct-public-admin-login-fixed'}));
...
app.use((req,res)=>res.status(404).json({success:false,message:'Endpoint not found',path:req.originalUrl,version:'auth-direct-v21'})); app.use(error); module.exports=app;
