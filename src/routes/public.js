...
router.get('/payment/options',ah(async(req,res)=>{const settings=await paymentSettings.getPublicSettings(); ok(res,{...settings, online:settings.onlinePaymentEnabled, providers:settings.razorpayConfigured?['RAZORPAY']:[]});}));
router.get(['/payments/settings','/payment/settings'],ah(async(req,res)=>{ok(res,await paymentSettings.getPublicSettings());}));
router.get('/platform/settings',ah(async(req,res)=>{ok(res,await paymentSettings.getPublicSettings());}));
