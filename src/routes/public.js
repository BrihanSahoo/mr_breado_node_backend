...
function razorpayConfigured(){ return !!(process.env.RAZORPAY_KEY_ID && process.env.RAZORPAY_KEY_SECRET && String(process.env.RAZORPAY_KEY_ID).startsWith('rzp_')); }
function publicRazorpayKey(row){ return row?.razorpay_key_id || process.env.RAZORPAY_KEY_ID || ''; }
...
router.get('/payment/options',ah(async(req,res)=>{const s=await safeOne('SELECT * FROM platform_settings ORDER BY id DESC LIMIT 1'); const configured=razorpayConfigured(); const online=s?bitBool(s.online_payment_enabled,false):configured; ok(res,{cod:true,codEnabled:s?bitBool(s.cod_enabled,true):true,online:online&&configured,onlinePaymentEnabled:online&&configured,razorpayEnabled:online&&configured,razorpayConfigured:configured,providers:configured?['RAZORPAY']:[],currency:'INR',razorpayKeyId:publicRazorpayKey(s)});}));
router.get(['/payments/settings','/payment/settings'],ah(async(req,res)=>{const s=await safeOne('SELECT * FROM payment_settings ORDER BY id DESC LIMIT 1'); const configured=razorpayConfigured(); const online=s?bitBool(s.online_payment_enabled,false):configured; ok(res,{codEnabled:s?bitBool(s.cod_enabled,true):true,onlineEnabled:online&&configured,onlinePaymentEnabled:online&&configured,razorpayEnabled:online&&configured,razorpayConfigured:configured,currency:'INR',razorpayKeyId:publicRazorpayKey(s),razorpayMode:s?.razorpay_mode||process.env.RAZORPAY_MODE||'TEST'});}));
router.get('/platform/settings',ah(async(req,res)=>{const s=await safeOne('SELECT * FROM platform_settings ORDER BY id DESC LIMIT 1'); const configured=razorpayConfigured(); if(!s) return ok(res,{platformFee:5,deliveryBaseFee:30,currency:'INR',autoCancelMinutes:60,codEnabled:true,onlinePaymentEnabled:configured,onlineEnabled:configured,razorpayEnabled:configured,razorpayConfigured:configured,razorpayKeyId:process.env.RAZORPAY_KEY_ID||'',mrBreadoTakeawayEnabled:true,takeawayOnlineRequired:false,takeawayBookingFeePercent:20}); const online=bitBool(s.online_payment_enabled,false); ok(res,{...s,codEnabled:bitBool(s.cod_enabled,true),onlinePaymentEnabled:online&&configured,onlineEnabled:online&&configured,razorpayEnabled:online&&configured,razorpayConfigured:configured,razorpayKeyId:publicRazorpayKey(s),razorpayMode:s.razorpay_mode||process.env.RAZORPAY_MODE||'TEST',mrBreadoTakeawayEnabled:bitBool(s.mr_breado_takeaway_enabled,false),takeawayOnlineRequired:false,takeawayBookingFeePercent:Number(s.takeaway_booking_fee_percent||20),deliveryChargePerKm:Number(s.delivery_charge_per_km||0),minimumDeliveryCharge:Number(s.minimum_delivery_charge||0),maximumDeliveryCharge:Number(s.maximum_delivery_charge||0),currency:'INR',autoCancelMinutes:60});}));
