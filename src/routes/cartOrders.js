...
router.post('/user/orders',ah(async(req,res)=>{
  ...
  const paymentStatus = paymentType==='ONLINE' && razorpayOrderId && razorpayPaymentId && razorpaySignature ? 'PAID' : (paymentType==='ONLINE'?'PENDING':'COD_PENDING');
  ...
  if(paymentType==='ONLINE' && razorpayOrderId){
    await exec('UPDATE payment_transactions SET order_id=:orderId, status=CASE WHEN status="CAPTURED" THEN "CAPTURED" ELSE status END WHERE provider_order_id=:providerOrderId',{orderId:r.insertId,providerOrderId:razorpayOrderId});
  }
...
router.post('/payments/create-order',ah(async(req,res)=>ok(res,await payment.createOrder({amount:req.body.amount||req.body.amountRupees||req.body.total||req.body.payableAmount,amountInPaise:req.body.amountInPaise||req.body.amount_in_paise,orderId:req.body.orderId||req.body.appOrderId,userId:req.user.id,currency:req.body.currency||'INR'}), 'Payment order created')));
router.post('/payments/verify',ah(async(req,res)=>ok(res,await payment.verify(req.body),'Payment verified')));
